...
<head>
  ...
  <script type="text/javascript">
    ...
    window.onload = function () {
      ...
      $("#datagrid").datagrid("options").url = "${path}/orderconsumesub/datagrid?" + $("#fmorder").serialize();
      ...
      <%--$.ajax({--%>
      <%--type: "POST",--%>
      <%--url: "${path}/orderconsumesub/getConfigSource?orderAction=" + "消费",--%>
      <%--cache: false,--%>
      <%--dataType: "json",--%>
    ...
    function getDealchannel() {
      ...
      $.ajax({
        type: "POST",
        url: "${path}/orderconsumesub/getDealChannel",
        cache: false,
        async: true,
        dataType: "json",
        success: function (data) {
    ...
    function getPageDisplay() {
      ...
      $.ajax({
        type: "POST",
        url: "${path}/orderconsumesub/getPageDisplay",
    ...
    function manualUpdate() {
      ...
      if (row) {
        ...
        url = path + "/orderconsumesub/manualUpdate";
        mesTitle = '手动处理成功';
        if(0 == row.sucFlag) {
          $("#dealResult").show();
          $("#points").hide();
...
<div region="center" border="false" style="overflow: hidden;">
  ...
  <%--url="${path}/orderconsumesub/datagrid" --%>
    ...
    <form id="fmorder" name="fmorder" method="post">
      ...
      <div style="padding-top:6px;padding-bottom: 6px;padding-left: 6px;padding-right: 6px">
        ...
        <input id="sourceSubchannel" name="sourceSubchannel" style="width: 155px" class="easyui-combobox" data-options="panelHeight:'auto', editable:false,valueField:'id',textField:'text',
					data:[{id:'',text:'全部'},{id:'harbor',text:'借款2.0(harbor)'},{id:'loan',text:'借款1.0(loan)'},
