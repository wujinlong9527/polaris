...
window.onload = function () {
    ...
    $("#datagrid").datagrid("options").url = getRootPath()+"/orderconsume/datagrid?" + $("#fmorder").serialize();
...
function getDealchannel() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath()+"/orderconsume/getDealChannel",
        cache: false,
        async: true,
        dataType: "json",
...
function getPageDisplay() {
    ...
    $.ajax({
        type: "POST",
        url: getRootPath()+"/orderconsume/getPageDisplay",
...
function manualUpdate() {
    ...
    if (row) {
        ...
        url = getRootPath() + "/orderconsume/manualUpdate";
        mesTitle = '手动处理成功';
        if(0 == row.sucFlag) {
            $("#dealResult").show();
            $("#points").hide();
