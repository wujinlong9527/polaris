...
function addNav(data) {
    ...
    $.each(data, function (i, sm) {
        ...
        $.each(sm.menus, function (j, o) {
            menulist += '<li><div><a ref="' + o.menuid + '" onclick="addTab1(\'' + o.url + '\',\'' + o.menuname + '\',\'' + o.button + '\')" href="javascript:void(0)"  '
                + '" style="font-size: 12px;text-decoration: underline;"><span class="icon ' + o.icon
...
function addTab1(url, text, button) {
    if (url) {
        var src = path + url + '?menu_button=' + button;
        if (!$.startWith(url, '/')) {	//不是本项目的url，例如www.baidu.com
            src = url;
        }
